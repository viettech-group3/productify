...
const TodayModal = () => {   //This file is used to fetch today task and show it on the screen
  ...
  useEffect(() => {   //Fetch Events of currentUser by getAllEvents() when we open <TodayModal/>
    axios
  ...
  const filterEventsByToday = eventsData => {  //Filter event in today
    ...
    >
      <div className={styles.taskBoard}>
        {filteredEvents.map(event => {
          return (
            <div>
              Event Name: {event.name} - Event Start: {event.start} - Event End:{event.end}
